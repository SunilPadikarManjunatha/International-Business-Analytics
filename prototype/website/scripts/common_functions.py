...
intersectOpportunities_skill = []
intersectOpportunities_back = []
...
def get_skill_associate(search_skills,skills_pref):
    filter_skills = dict()
    for skill in search_skills.split(','):
    ...
    if str(row[col_name]) != 'nan':
        ...
        while i<len(skills):
            if skills[i] in skills_intersect:
                ...
                i+=1
        if col_name == "opp_skill_pref":
            intersectOpportunities_skill.append(",".join(rated_skills))
        else:
            intersectOpportunities_back.append(",".join(rated_skills))
    else:
        if col_name == "opp_skill_pref":
            intersectOpportunities_skill.append("")
        else:
            intersectOpportunities_back.append("")
